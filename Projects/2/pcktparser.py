...
def printPacketsV4(filter, data, raw_data):
    (version, header_length, ttl, proto, src, target, data, is_fragmented) = ipv4_Packet(data)
    ...
    return proto, src, len(data), is_fragmented
...
def printPacketsV6(filter, nextProto, newPacket):
    ...
    return remainingPacket, proto, src, len(remainingPacket)
...
def icmpv6Header(data):
    ipv6_icmp_type, ipv6_icmp_code, ipv6_icmp_chekcsum = struct.unpack(">BBH", data[:4])
...
def ipv4_Packet(data):
    ...
    check_fragment = bin(data[7])
    if check_fragment == 1:
        frag = True
    ttl, proto, src, target = struct.unpack('! 8x B B 2x 4s 4s', data[:20])
    return version, header_len, ttl, proto, ipv4(src), ipv4(target), data[header_len:], frag
